...
median for each variable. The resulting contingency table was tested
using a chi-square test of independence to assess whether the observed
distribution differed from what would be expected if the variables were
unrelated.
...
negative correlation did not present in these data. The chi-square test
